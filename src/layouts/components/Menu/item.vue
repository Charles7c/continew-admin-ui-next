...
<template>
  <div
    ref="itemRef" class="menu-item relative py-1 transition-all" :class="{
      'active': isItemActive,
      'px-2': settingsStore.settings.menu.isRounded,
    }"
  >
    <router-link v-slot="{ href, navigate }" custom :to="uniqueKey.at(-1) ?? ''">
      <HTooltip :enable="rootMenu.isMenuPopup && level === 0 && !subMenu" :text="typeof item.meta?.title === 'function' ? item.meta?.title() : item.meta?.title" placement="right" class="h-full w-full">
        ...
        >
          ...
          >
            <SvgIcon v-if="props.item.meta?.icon" :name="props.item.meta.icon" :size="20" class="menu-item-container-icon transition-transform" async />
            <span
